...
if getattr(sys, 'frozen', False):
    _FD_BASE_DIR = os.path.join(sys._MEIPASS, "3rd_party", "file_dialog")
else:
    _FD_BASE_DIR = os.path.dirname(__file__)
...
class FileDialog:
    ...
    def _sort_items(self, items):
        """Sort a list of file/dir names based on current sort state."""
        col = self._sort_column
        rev = self._sort_reverse
        try:
            if col == 0:  # name
                items.sort(key=lambda x: os.path.basename(x).lower(), reverse=rev)
            elif col == 1:  # date
                items.sort(key=lambda x: os.path.getctime(x), reverse=rev)
            elif col == 2:  # type
                items.sort(key=lambda x: os.path.splitext(x)[1].lower(), reverse=rev)
            elif col == 3:  # size
                items.sort(key=lambda x: os.path.getsize(x) if os.path.isfile(x) else 0, reverse=rev)
        except Exception:
            pass
        return items
    ...
    def _on_sort(self, sender, sort_specs):
        ...
        direction = sort_specs[0][1]  # negative = descending

        # Map column tag to index
        col_map = {
            self.tag + "ex_name": 0,
            self.tag + "ex_date": 1,
            self.tag + "ex_type": 2,
        ...
        self._sort_column = col_map.get(col_id, 0)
        self._sort_reverse = (direction < 0)
        self.reset_dir(default_path=os.getcwd())
    ...
    def reset_dir(self, file_name_filter=None, default_path=None):
        ...
        def internal():
            ...
            try:
                ...
                dirs = [f for f in _dir if os.path.isdir(f)]
                files = [f for f in _dir if os.path.isfile(f)]

                # Apply sort
                dirs = self._sort_items(dirs)
    ...
    def __init__(
        ...
        # sort state
        self._sort_column = 0   # 0=name, 1=date, 2=type, 3=size
        self._sort_reverse = False

        self.fd_img_path = os.path.join(_FD_BASE_DIR, "images")
        ...
        with dpg.window(label="File dialog", tag=self.tag, no_resize=self.no_resize, show=False, modal=self.modal, width=self.width, height=self.height, min_size=self.min_size, no_collapse=True, pos=(50, 50)):
            ...
            with dpg.group(horizontal=True):
                '''
                with dpg.table(tag="File type filter_table", header_row=False, 
                                  borders_innerH=False, borders_innerV=False, borders_outerH=False, borders_outerV=False):
                        dpg.add_table_column(label="Spacer1_column", width_stretch=True, init_width_or_weight=80)
                        dpg.add_table_column(label="Text_column", width_fixed=True, init_width_or_weight=80)
                        dpg.add_table_column(label="Combo_column", width_fixed=True, init_width_or_weight=200)
                        with dpg.table_row():
                            with dpg.table_cell():
                            ...
                            with dpg.table_cell():
                                dpg.add_combo(items=self.filter_list,
                                              callback=self.filter_combo_selector, default_value=self.file_filter, width=200)
                '''

            with dpg.group(horizontal=True) as btn_ret_grp:
                with dpg.table(tag=self.tag+"button_table", header_row=False, 
                                  borders_innerH=False, borders_innerV=False, borders_outerH=False, borders_outerV=False):
                        dpg.add_table_column(label="Spacer2_column", width_stretch=True, init_width_or_weight=80)
                        dpg.add_table_column(label="OK_column", width_fixed=True, init_width_or_weight=80)
                        dpg.add_table_column(label="Cancel_column", width_fixed=True, init_width_or_weight=80)
                        with dpg.table_row():
                            with dpg.table_cell():
                                dpg.add_spacer(width=-1)
                            with dpg.table_cell():
                                dpg.add_button(label="   OK   ", tag=self.tag + "_return", callback=self.return_items, width = 80)
                            with dpg.table_cell():
                                dpg.add_button(label=" Cancel ",
                                               callback=lambda: dpg.hide_item(self.tag),  width = 80)
    ...
    def show_file_dialog(self):
        ...
        try:
            dpg.split_frame()  # ensure size is computed
            viewport_width = dpg.get_viewport_client_width()
            viewport_height = dpg.get_viewport_client_height()

            win_width = dpg.get_item_width(self.tag) or self.width
            win_height = dpg.get_item_height(self.tag) or self.height

            x = max(0, int(viewport_width // 2 - win_width // 2))
            y = max(0, int(viewport_height // 2 - win_height // 2))
            dpg.set_item_pos(self.tag, [x, y])
        except Exception:
            # If viewport isn't ready yet, keep default position
            pass
